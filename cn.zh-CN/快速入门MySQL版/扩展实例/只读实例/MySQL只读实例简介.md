# MySQL只读实例简介 {#concept_cst_z45_vdb .concept}
...
创建只读实例时会从备实例复制数据，数据与主实例一致，主实例的数据更新也会在主实例完成操作后立即自动同步到所有只读实例，也可以在只读实例上设置[只读实例延时复制](../../../../intl.zh-CN/RDS for MySQL用户指南/实例管理/只读实例延时复制.md#)。
...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/7826/15450320466089_zh-CN.png)
